...
// NEW: Refresh Token Schema (for session management)
export interface RefreshTokenProps extends Document {
  owner: Types.ObjectId; // Reference to User._id
  token: string; // The actual refresh token string
  expires: Date; // When this token expires
  createdIp?: string; // IP address that created this token
  createdAt?: Date;
}
